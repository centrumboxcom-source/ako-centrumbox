...
import { ArticleLessonEditor } from "@/components/course-builder/ArticleLessonEditor";
...
export default function CourseBuilderPage() {
  ...
  const [viewMode, setViewMode] = useState<"writer" | "split" | "preview">("writer");
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col">
      {/* Top Navigation Bar */}
      <header className="h-16 px-4 sm:px-6 bg-white/95 border-b border-slate-200 flex items-center justify-between sticky top-0 z-30 backdrop-blur-md shadow-sm">
        <div className="flex items-center gap-3 sm:gap-4">
          <Link
            href={tenantSubdomain ? `/admin?tenant=${tenantSubdomain}` : "/admin"}
            className="flex items-center gap-2 text-xs font-semibold text-slate-600 hover:text-slate-900 transition px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 border border-slate-200"
          >
            <ArrowLeft className="h-3.5 w-3.5" />
            <span className="hidden sm:inline">До панелі керування</span>
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs px-2.5 py-0.5 rounded-lg bg-indigo-50 border border-indigo-200 text-indigo-700 font-mono font-bold">
                {tenantSubdomain}
              </span>
              <h1 className="text-sm sm:text-base font-bold text-slate-900 truncate max-w-[180px] sm:max-w-md">
          ...
          {activeLessonId && (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-1.5 rounded-xl text-xs font-bold transition shadow-sm ${
                hasChanges
                  ? "bg-indigo-600 hover:bg-indigo-700 text-white shadow-indigo-600/20 ring-2 ring-indigo-300 ring-offset-1 animate-pulse"
            ...
            >
              {savingLesson ? (
                ...
                <>
                  ...
                  {hasChanges ? "Зберегти статтю" : "Збережено"}
                </>
              )}
            </button>
          )}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-72 sm:w-80 border-r border-slate-200 bg-white p-4 flex flex-col shrink-0 space-y-4 shadow-sm">
          {/* Tab Switcher */}
            ...
            <div className="flex flex-col h-full space-y-4">
              <div className="flex items-center justify-between pb-2 border-b border-slate-200">
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold flex items-center gap-1 transition shadow-sm"
                >
              ...
              <div className="flex-1 overflow-y-auto space-y-2 pr-1">
                {quizzes.length === 0 ? (
                  ...
                  quizzes.map((q) => (
                    <div
                      key={q.id}
                      className="p-3 rounded-xl bg-slate-50/70 border border-slate-200 hover:border-slate-300 transition flex flex-col gap-2"
                    >
                      ...
                      <Link
                        ...
                        className="w-full py-1.5 px-2 rounded-lg bg-white hover:bg-indigo-600 hover:text-white text-slate-700 text-xs font-semibold flex items-center justify-center gap-1 transition border border-slate-200 shadow-sm"
                      >
        ...
        {/* Right Column: Active Lesson Article Editor */}
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 bg-slate-50/60">
          <div className="max-w-6xl mx-auto space-y-6">
            ...
            {activeLessonId ? (
              ...
              <div className="py-24 text-center border-2 border-dashed border-slate-200 rounded-3xl bg-white/70 p-8 shadow-sm max-w-xl mx-auto space-y-3">
                <BookOpen className="h-12 w-12 text-slate-400 mx-auto" />
                <h3 className="text-lg font-bold text-slate-900">Оберіть або створіть урок</h3>
                <p className="text-xs sm:text-sm text-slate-500 max-w-sm mx-auto">
                  Оберіть урок із програми ліворуч або натисніть «Додати новий урок», щоб перейти до створення навчальної статті.
                </p>
              </div>
            )}
          </div>
            ...
            <form onSubmit={handleCreateQuiz} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-900 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-indigo-500"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-900 font-mono focus:bg-white focus:outline-none focus:border-indigo-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-sm text-amber-600 font-mono font-bold focus:bg-white focus:outline-none focus:border-indigo-500"
                  />
                </div>
              </div>
              ...
              <div className="pt-2 flex justify-end gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold transition disabled:opacity-50 shadow-sm"
                >
